 ...
 */
function bashback(err:Error, stdout:string, stderr:string) {
	if (err) {
 ...
 */
function bat(command:string, callback=bashback):child_process.ChildProcess {
 ...
 * @returns {User} user
 */
async function getUser(usernameEmail):Promise<User> {
